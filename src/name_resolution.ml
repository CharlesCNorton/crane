...
(** Classify an inductive type using only side-effect-free queries. We use the
    raw ip_typename from the packet and the ind_kind from miniml to compute
    classification flags without calling pp_global_name. *)
