...
(** Reset all C++ extraction global state. MUST be called between separate
    extractions to avoid state pollution when running multiple extractions in
    the same process (e.g., during 'dune build'). Clears: struct context,
    template context, method candidates, method registry, etc. *)
