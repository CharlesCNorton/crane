...
type cpp_name = {
  ...
  cn_qualified : string option;   (** Optional qualifier prefix, e.g., Some "Nat::" *)
  cn_needs_typename : bool;       (** True if dependent type requires typename keyword in template context *)
}
...
type cpp_ind_kind =
  ...
  | IK_Record of GlobRef.t option list      (** Product type rendered as struct, with field references *)
  | IK_Eponymous of GlobRef.t option list   (** Record merged into its module struct to avoid naming conflicts *)
  | IK_TypeClass of GlobRef.t option list   (** Type class rendered as C++ concept *)
...
type custom_info = {
  ci_inline : string option;   (** Some code if entity should be inlined, None otherwise *)
...
type cpp_type =
  | Tvar of int * Id.t option                    (** Type variable with De Bruijn index and optional name *)
  | Tid of Id.t * cpp_type list                  (** Local type identifier with type arguments, for nested structs *)
  | Tglob of GlobRef.t * cpp_type list * cpp_expr list  (** Global type reference with type and value arguments *)
  | Tfun of cpp_type list * cpp_type             (** Function type: domain types and codomain *)
  | Tmod of cpp_tymod * cpp_type                 (** Type with modifier (const, static, extern) *)
  | Tnamespace of GlobRef.t * cpp_type           (** Type qualified by namespace reference *)
  | Tqualified of cpp_type * Id.t                (** Nested type access, e.g., typename Base<T>::nested *)
...
and cpp_meta = {
  ...
  mutable contents : cpp_type option  (** Unification result, None if unresolved *)
}
...
and cpp_stmt =
  ...
  | Sasgn of Id.t * cpp_type option * cpp_expr    (** Variable assignment with optional type annotation *)
  ...
  | Scustom_case of cpp_type * cpp_expr * cpp_type list * ((Id.t * cpp_type) list * cpp_type * cpp_stmt list) list * string
      (** Custom pattern match: return type, scrutinee, type args, branches (params, type, body), custom match string *)
  | Sthrow of string                              (** Throw exception with message, for unreachable/absurd cases *)
  | Sswitch of cpp_expr * GlobRef.t * (Id.t * cpp_stmt list) list
      (** Switch statement: scrutinee, enum type reference, branches (constructor, body) *)
  | Sassert of string * string option             (** Runtime assertion: C++ condition, optional Rocq predicate comment *)
  | Sif of cpp_expr * cpp_stmt list * cpp_stmt list
      (** Conditional: condition, then-branch, else-branch (used for reuse optimization) *)
  | Sraw of string                                (** Raw C++ code printed verbatim *)
  | Sassign_field of cpp_expr * Id.t * cpp_expr   (** Field assignment for in-place mutation during memory reuse *)
...
and cpp_expr =
  ...
  | CPPglob of GlobRef.t * cpp_type list * custom_info option
      (** Global reference with type arguments and optional custom extraction info *)
  | CPPnamespace of GlobRef.t * cpp_expr          (** Namespace-qualified expression *)
  | CPPfun_call of cpp_expr * cpp_expr list       (** Function call with arguments (stored in reverse order) *)
  | CPPderef of cpp_expr                          (** Pointer dereference *)
  | CPPmove of cpp_expr                           (** std::move for move semantics *)
  | CPPforward of cpp_type * cpp_expr             (** std::forward<T> for perfect forwarding *)
  | CPPlambda of (cpp_type * Id.t option) list * cpp_type option * cpp_stmt list * bool
  ...
  | CPPoverloaded of cpp_expr list                (** Overloaded visitor set for variant matching *)
  ...
  | CPPparray of cpp_expr array * cpp_expr        (** Persistent array with element array and default value *)
  | CPPrequires of (cpp_type * Id.t) list * (cpp_expr * cpp_constraint) list * cpp_type list
      (** Requires expression: parameters, expression-constraint pairs, type requirements *)
  | CPPnew of cpp_type * cpp_expr list            (** Heap allocation: new Type(args) *)
  | CPPshared_ptr_ctor of cpp_type * cpp_expr     (** Direct std::shared_ptr<T>(expr) construction *)
  | CPPunique_ptr_ctor of cpp_type * cpp_expr     (** Direct std::unique_ptr<T>(expr) construction *)
  | CPPmk_unique of cpp_type                      (** std::make_unique<T> factory function *)
  | CPPthis                                       (** this pointer in method context *)
  | CPPshared_from_this of cpp_type               (** std::const_pointer_cast<T>(shared_from_this()) *)
  | CPPmember of cpp_expr * Id.t                  (** Member access with dot operator: expr.member *)
  | CPParrow of cpp_expr * Id.t                   (** Member access with arrow operator: expr->member *)
  ...
  | CPPbinop of string * cpp_expr * cpp_expr      (** Binary operator for reuse optimization conditions *)
...
and template_type =
  ...
  | TTtypename_default of cpp_type            (** typename with default: typename T = default_type *)
  | TTfun of (cpp_type list * cpp_type)       (** Function type parameter for higher-order templates *)
...
val map_expr : (cpp_expr -> cpp_expr) -> (cpp_stmt -> cpp_stmt) -> (cpp_type -> cpp_type) -> cpp_expr -> cpp_expr

(** [map_stmt fe fs ft s] applies [fe] to sub-expressions, [fs] to sub-statements,
    [ft] to sub-types, performing one level of structural descent. *)
val map_stmt : (cpp_expr -> cpp_expr) -> (cpp_stmt -> cpp_stmt) -> (cpp_type -> cpp_type) -> cpp_stmt -> cpp_stmt
...
type cpp_decl =
  | Dtemplate of (template_type * Id.t) list * cpp_constraint option * cpp_decl
      (** Template declaration: parameters, optional constraint, inner declaration *)
  | Dnspace of GlobRef.t option * cpp_decl list   (** Namespace with optional reference and declarations *)
  | Dfundef of (GlobRef.t * cpp_type list) list * cpp_type * (Id.t * cpp_type) list * cpp_stmt list
      (** Function definition: names with type args, return type, parameters, body *)
  | Dfundecl of (GlobRef.t * cpp_type list) list * cpp_type * (Id.t option * cpp_type) list
      (** Function declaration: names with type args, return type, parameters (may be unnamed) *)
  | Dstruct of {
      ds_ref : GlobRef.t;                          (** Struct reference *)
      ds_fields : (cpp_field * cpp_visibility) list;  (** Fields with visibility *)
      ds_tparams : (template_type * Id.t) list;    (** Template parameters (empty for non-templates) *)
      ds_constraint : cpp_constraint option;       (** Optional template constraint *)
      ds_needs_shared_from_this : bool;            (** True if inherits enable_shared_from_this *)
    }
  | Dstruct_decl of GlobRef.t                     (** Forward struct declaration *)
  | Dusing of GlobRef.t * cpp_type                (** Type alias: using name = type *)
  | Dasgn of GlobRef.t * cpp_type * cpp_expr      (** Global variable definition with initializer *)
  | Ddecl of GlobRef.t * cpp_type                 (** Global variable declaration *)
  | Dconcept of GlobRef.t * cpp_expr              (** Concept definition (template params from outer Dtemplate) *)
  | Dstatic_assert of cpp_expr * string option    (** Static assertion with optional message *)
  | Denum of {
