#include <algorithm>
#include <any>
#include <branch_target_in_page_prop.h>
#include <cassert>
#include <functional>
#include <iostream>
#include <memory>
...
unsigned int BranchTargetInPageProp::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int BranchTargetInPageProp::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int BranchTargetInPageProp::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int BranchTargetInPageProp::branch_target(const unsigned int pc,
