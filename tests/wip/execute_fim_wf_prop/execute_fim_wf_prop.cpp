...
unsigned int
ExecuteFimWfProp::get_reg(const std::shared_ptr<ExecuteFimWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteFimWfProp::set_reg(const std::shared_ptr<ExecuteFimWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteFimWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteFimWfProp::get_pair(const std::shared_ptr<ExecuteFimWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteFimWfProp::set_pair(const std::shared_ptr<ExecuteFimWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteFimWfProp::push_return(std::shared_ptr<ExecuteFimWfProp::state> s,
...
std::shared_ptr<ExecuteFimWfProp::state>
ExecuteFimWfProp::execute(std::shared_ptr<ExecuteFimWfProp::state> s,
                          const std::shared_ptr<ExecuteFimWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteFimWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFimWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteFimWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteFimWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteFimWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteFimWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFimWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFimWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFimWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteFimWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            return std::make_shared<ExecuteFimWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteFimWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteFimWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteFimWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
          ...
          [&](const typename ExecuteFimWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteFimWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteFimWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(
          ...
          [&](const typename ExecuteFimWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFimWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteFimWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteFimWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteFimWfProp::state> {
