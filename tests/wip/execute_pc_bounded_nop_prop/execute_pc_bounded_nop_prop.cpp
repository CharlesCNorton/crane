...
#include <any>
#include <cassert>
#include <execute_pc_bounded_nop_prop.h>
#include <functional>
#include <iostream>
#include <memory>
#include <optional>
...
unsigned int ExecutePcBoundedNopProp::get_reg(
    const std::shared_ptr<ExecutePcBoundedNopProp::state> &s,
    const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>> ExecutePcBoundedNopProp::set_reg(
    const std::shared_ptr<ExecutePcBoundedNopProp::state> &s,
    const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecutePcBoundedNopProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int ExecutePcBoundedNopProp::get_pair(
    const std::shared_ptr<ExecutePcBoundedNopProp::state> &s,
    const unsigned int r) {
...
std::shared_ptr<List<unsigned int>> ExecutePcBoundedNopProp::set_pair(
    const std::shared_ptr<ExecutePcBoundedNopProp::state> &s,
    const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>> ExecutePcBoundedNopProp::push_return(
    std::shared_ptr<ExecutePcBoundedNopProp::state> s, const unsigned int ret) {
  return List<unsigned int>::ctor::cons_((std::move(ret) % 4096u),
                                         std::move(s)->ex_stack)
      ->firstn(2u);
}

std::shared_ptr<ExecutePcBoundedNopProp::state>
ExecutePcBoundedNopProp::execute(
    std::shared_ptr<ExecutePcBoundedNopProp::state> s,
    const std::shared_ptr<ExecutePcBoundedNopProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecutePcBoundedNopProp::instr::NOP _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedNopProp::instr::LDM _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecutePcBoundedNopProp::instr::LD _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::XCH _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::INC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::ADD _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::SUB _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::IAC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::DAC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::CLC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedNopProp::instr::STC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedNopProp::instr::CMC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::CMA _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::CLB _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedNopProp::instr::RAL _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::RAR _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::TCC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::TCS _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::DAA _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::KBP _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::JUN _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::JMS _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::JCN _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::FIM _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::SRC _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::FIN _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::JIN _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedNopProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::ISZ _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecutePcBoundedNopProp::instr::BBL _args)
              -> std::shared_ptr<ExecutePcBoundedNopProp::state> {
