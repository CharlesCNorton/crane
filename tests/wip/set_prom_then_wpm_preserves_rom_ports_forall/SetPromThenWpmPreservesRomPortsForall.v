...
(* Behavioral candidate: set_prom then WPM preserves ROM port bounds. *)

From Stdlib Require Import List Nat Bool.
Import ListNotations.

Module SetPromThenWpmPreservesRomPortsForall.
...
Definition after : state := execute_wpm (set_prom_params sample 3 99 true).
Definition t : Prop := Forall (fun x => x < 16) (rom_ports after).

End SetPromThenWpmPreservesRomPortsForall.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "set_prom_then_wpm_preserves_rom_ports_forall" SetPromThenWpmPreservesRomPortsForall.
