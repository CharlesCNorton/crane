#include <addr_space_wrap_prop.h>
#include <algorithm>
#include <any>
#include <cassert>
#include <functional>
...
unsigned int AddrSpaceWrapProp::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int AddrSpaceWrapProp::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int AddrSpaceWrapProp::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int AddrSpaceWrapProp::branch_target(const unsigned int pc,
