...
#include <any>
#include <cassert>
#include <execute_pc_bounded_bbl_prop.h>
#include <functional>
#include <iostream>
#include <memory>
#include <optional>
...
unsigned int ExecutePcBoundedBblProp::get_reg(
    const std::shared_ptr<ExecutePcBoundedBblProp::state> &s,
    const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>> ExecutePcBoundedBblProp::set_reg(
    const std::shared_ptr<ExecutePcBoundedBblProp::state> &s,
    const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecutePcBoundedBblProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int ExecutePcBoundedBblProp::get_pair(
    const std::shared_ptr<ExecutePcBoundedBblProp::state> &s,
    const unsigned int r) {
...
std::shared_ptr<List<unsigned int>> ExecutePcBoundedBblProp::set_pair(
    const std::shared_ptr<ExecutePcBoundedBblProp::state> &s,
    const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>> ExecutePcBoundedBblProp::push_return(
    std::shared_ptr<ExecutePcBoundedBblProp::state> s, const unsigned int ret) {
  return List<unsigned int>::ctor::cons_((std::move(ret) % 4096u),
                                         std::move(s)->ex_stack)
      ->firstn(2u);
}

std::shared_ptr<ExecutePcBoundedBblProp::state>
ExecutePcBoundedBblProp::execute(
    std::shared_ptr<ExecutePcBoundedBblProp::state> s,
    const std::shared_ptr<ExecutePcBoundedBblProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecutePcBoundedBblProp::instr::NOP _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedBblProp::instr::LDM _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecutePcBoundedBblProp::instr::LD _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::XCH _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::INC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::ADD _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::SUB _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::IAC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::DAC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::CLC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedBblProp::instr::STC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedBblProp::instr::CMC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::CMA _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::CLB _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecutePcBoundedBblProp::instr::RAL _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::RAR _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::TCC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::TCS _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::DAA _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::KBP _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::JUN _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::JMS _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::JCN _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::FIM _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::SRC _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::FIN _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::JIN _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecutePcBoundedBblProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::ISZ _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecutePcBoundedBblProp::instr::BBL _args)
              -> std::shared_ptr<ExecutePcBoundedBblProp::state> {
