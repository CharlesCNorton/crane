...
#include <memory>
#include <optional>
#include <push_return_bound_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>

unsigned int PushReturnBoundProp::get_reg(
    const std::shared_ptr<PushReturnBoundProp::state> &s,
    const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>> PushReturnBoundProp::set_reg(
    const std::shared_ptr<PushReturnBoundProp::state> &s, const unsigned int r,
    const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int PushReturnBoundProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int PushReturnBoundProp::get_pair(
    const std::shared_ptr<PushReturnBoundProp::state> &s,
    const unsigned int r) {
...
std::shared_ptr<List<unsigned int>> PushReturnBoundProp::set_pair(
    const std::shared_ptr<PushReturnBoundProp::state> &s, const unsigned int r,
    const unsigned int v) {
  unsigned int base = pair_base(r);
...
PushReturnBoundProp::push_return(std::shared_ptr<PushReturnBoundProp::state> s,
...
std::shared_ptr<PushReturnBoundProp::state> PushReturnBoundProp::execute(
    std::shared_ptr<PushReturnBoundProp::state> s,
    const std::shared_ptr<PushReturnBoundProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename PushReturnBoundProp::instr::NOP _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnBoundProp::instr::LDM _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename PushReturnBoundProp::instr::LD _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::XCH _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename PushReturnBoundProp::instr::INC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::ADD _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename PushReturnBoundProp::instr::SUB _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename PushReturnBoundProp::instr::IAC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::DAC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::CLC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnBoundProp::instr::STC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnBoundProp::instr::CMC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::CMA _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::CLB _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnBoundProp::instr::RAL _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::RAR _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::TCC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename PushReturnBoundProp::instr::TCS _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            return std::make_shared<PushReturnBoundProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename PushReturnBoundProp::instr::DAA _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename PushReturnBoundProp::instr::KBP _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::JUN _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::JMS _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename PushReturnBoundProp::instr::JCN _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
          ...
          [&](const typename PushReturnBoundProp::instr::FIM _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename PushReturnBoundProp::instr::SRC _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename PushReturnBoundProp::instr::FIN _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(
          ...
          [&](const typename PushReturnBoundProp::instr::JIN _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnBoundProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename PushReturnBoundProp::instr::ISZ _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename PushReturnBoundProp::instr::BBL _args)
              -> std::shared_ptr<PushReturnBoundProp::state> {
