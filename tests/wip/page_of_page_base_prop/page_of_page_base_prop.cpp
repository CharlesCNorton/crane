...
#include <memory>
#include <optional>
#include <page_of_page_base_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>

unsigned int PageOfPageBaseProp::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int PageOfPageBaseProp::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int PageOfPageBaseProp::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int PageOfPageBaseProp::branch_target(const unsigned int pc,
