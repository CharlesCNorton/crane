...
unsigned int
ExecuteLdmWfProp::get_reg(const std::shared_ptr<ExecuteLdmWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteLdmWfProp::set_reg(const std::shared_ptr<ExecuteLdmWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteLdmWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteLdmWfProp::get_pair(const std::shared_ptr<ExecuteLdmWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteLdmWfProp::set_pair(const std::shared_ptr<ExecuteLdmWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteLdmWfProp::push_return(std::shared_ptr<ExecuteLdmWfProp::state> s,
...
std::shared_ptr<ExecuteLdmWfProp::state>
ExecuteLdmWfProp::execute(std::shared_ptr<ExecuteLdmWfProp::state> s,
                          const std::shared_ptr<ExecuteLdmWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteLdmWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdmWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteLdmWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteLdmWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteLdmWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteLdmWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdmWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdmWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdmWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteLdmWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteLdmWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteLdmWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteLdmWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
          ...
          [&](const typename ExecuteLdmWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteLdmWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteLdmWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(
          ...
          [&](const typename ExecuteLdmWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdmWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteLdmWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteLdmWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteLdmWfProp::state> {
