...
(* Behavioral candidate: set_prom then WPM preserves register bounds. *)

From Stdlib Require Import List Nat Bool.
Import ListNotations.

Module SetPromThenWpmPreservesRegsForall.
...
Definition set_prom_params (s : state) (addr data : nat) (enable : bool) : state :=
  mkState (regs s) (rom s) addr data enable.
...
Definition t : Prop :=
  Forall (fun x => x < 16)
         (regs (execute_wpm (set_prom_params sample 1 99 true))).

End SetPromThenWpmPreservesRegsForall.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "set_prom_then_wpm_preserves_regs_forall" SetPromThenWpmPreservesRegsForall.
