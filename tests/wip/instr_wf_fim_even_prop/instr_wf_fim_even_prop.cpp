...
#include <cassert>
#include <functional>
#include <instr_wf_fim_even_prop.h>
#include <iostream>
#include <memory>
#include <optional>
#include <stdexcept>
...
unsigned int
InstrWfFimEvenProp::get_reg(const std::shared_ptr<InstrWfFimEvenProp::state> &s,
                            const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
InstrWfFimEvenProp::set_reg(const std::shared_ptr<InstrWfFimEvenProp::state> &s,
                            const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int InstrWfFimEvenProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int InstrWfFimEvenProp::get_pair(
    const std::shared_ptr<InstrWfFimEvenProp::state> &s, const unsigned int r) {
  unsigned int base = pair_base(r);
  return (((get_reg(s, base) % 16u) * 16u) + (get_reg(s, (base + 1)) % 16u));
}

std::shared_ptr<List<unsigned int>> InstrWfFimEvenProp::set_pair(
    const std::shared_ptr<InstrWfFimEvenProp::state> &s, const unsigned int r,
...
std::shared_ptr<List<unsigned int>>
InstrWfFimEvenProp::push_return(std::shared_ptr<InstrWfFimEvenProp::state> s,
...
std::shared_ptr<InstrWfFimEvenProp::state> InstrWfFimEvenProp::execute(
    std::shared_ptr<InstrWfFimEvenProp::state> s,
    const std::shared_ptr<InstrWfFimEvenProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename InstrWfFimEvenProp::instr::NOP _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimEvenProp::instr::LDM _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename InstrWfFimEvenProp::instr::LD _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::XCH _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfFimEvenProp::instr::INC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::ADD _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename InstrWfFimEvenProp::instr::SUB _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename InstrWfFimEvenProp::instr::IAC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::DAC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::CLC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimEvenProp::instr::STC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimEvenProp::instr::CMC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::CMA _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::CLB _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimEvenProp::instr::RAL _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::RAR _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::TCC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfFimEvenProp::instr::TCS _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfFimEvenProp::instr::DAA _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename InstrWfFimEvenProp::instr::KBP _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::JUN _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::JMS _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename InstrWfFimEvenProp::instr::JCN _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
          ...
          [&](const typename InstrWfFimEvenProp::instr::FIM _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfFimEvenProp::instr::SRC _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename InstrWfFimEvenProp::instr::FIN _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(
          ...
          [&](const typename InstrWfFimEvenProp::instr::JIN _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimEvenProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename InstrWfFimEvenProp::instr::ISZ _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename InstrWfFimEvenProp::instr::BBL _args)
              -> std::shared_ptr<InstrWfFimEvenProp::state> {
