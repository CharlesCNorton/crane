...
unsigned int
ExecuteTcsWfProp::get_reg(const std::shared_ptr<ExecuteTcsWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteTcsWfProp::set_reg(const std::shared_ptr<ExecuteTcsWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteTcsWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteTcsWfProp::get_pair(const std::shared_ptr<ExecuteTcsWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteTcsWfProp::set_pair(const std::shared_ptr<ExecuteTcsWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteTcsWfProp::push_return(std::shared_ptr<ExecuteTcsWfProp::state> s,
...
std::shared_ptr<ExecuteTcsWfProp::state>
ExecuteTcsWfProp::execute(std::shared_ptr<ExecuteTcsWfProp::state> s,
                          const std::shared_ptr<ExecuteTcsWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteTcsWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTcsWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteTcsWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteTcsWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteTcsWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteTcsWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTcsWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTcsWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTcsWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteTcsWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteTcsWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteTcsWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteTcsWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
          ...
          [&](const typename ExecuteTcsWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteTcsWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteTcsWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(
          ...
          [&](const typename ExecuteTcsWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTcsWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteTcsWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteTcsWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteTcsWfProp::state> {
