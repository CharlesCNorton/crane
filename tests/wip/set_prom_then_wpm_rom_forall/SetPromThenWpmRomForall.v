...
(* Behavioral candidate: set_prom then WPM preserves ROM byte bounds. *)

From Stdlib Require Import List Nat Bool.
Import ListNotations.

Module SetPromThenWpmRomForall.
...
Definition sample : state := mkState [10; 11; 12] 0 0 false.
Definition t : Prop :=
  Forall (fun x => x < 256)
         (rom (execute_wpm (set_prom_params sample 1 99 true))).

End SetPromThenWpmRomForall.

Require Crane.Extraction.
From Crane Require Mapping.Std Mapping.NatIntStd.
Crane Extraction "set_prom_then_wpm_rom_forall" SetPromThenWpmRomForall.
