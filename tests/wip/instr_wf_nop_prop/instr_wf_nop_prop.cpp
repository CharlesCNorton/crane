...
unsigned int
InstrWfNopProp::get_reg(const std::shared_ptr<InstrWfNopProp::state> &s,
                        const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
InstrWfNopProp::set_reg(const std::shared_ptr<InstrWfNopProp::state> &s,
                        const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int InstrWfNopProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
InstrWfNopProp::get_pair(const std::shared_ptr<InstrWfNopProp::state> &s,
                         const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
InstrWfNopProp::set_pair(const std::shared_ptr<InstrWfNopProp::state> &s,
                         const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
InstrWfNopProp::push_return(std::shared_ptr<InstrWfNopProp::state> s,
...
std::shared_ptr<InstrWfNopProp::state>
InstrWfNopProp::execute(std::shared_ptr<InstrWfNopProp::state> s,
                        const std::shared_ptr<InstrWfNopProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename InstrWfNopProp::instr::NOP _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfNopProp::instr::LDM _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename InstrWfNopProp::instr::LD _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::XCH _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfNopProp::instr::INC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::ADD _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename InstrWfNopProp::instr::SUB _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename InstrWfNopProp::instr::IAC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::DAC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::CLC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfNopProp::instr::STC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfNopProp::instr::CMC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::CMA _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::CLB _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfNopProp::instr::RAL _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::RAR _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::TCC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfNopProp::instr::TCS _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            return std::make_shared<InstrWfNopProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfNopProp::instr::DAA _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename InstrWfNopProp::instr::KBP _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::JUN _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::JMS _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename InstrWfNopProp::instr::JCN _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
          ...
          [&](const typename InstrWfNopProp::instr::FIM _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfNopProp::instr::SRC _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename InstrWfNopProp::instr::FIN _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(
          ...
          [&](const typename InstrWfNopProp::instr::JIN _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfNopProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename InstrWfNopProp::instr::ISZ _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename InstrWfNopProp::instr::BBL _args)
              -> std::shared_ptr<InstrWfNopProp::state> {
