...
unsigned int
ExecuteSubWfProp::get_reg(const std::shared_ptr<ExecuteSubWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteSubWfProp::set_reg(const std::shared_ptr<ExecuteSubWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteSubWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteSubWfProp::get_pair(const std::shared_ptr<ExecuteSubWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteSubWfProp::set_pair(const std::shared_ptr<ExecuteSubWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteSubWfProp::push_return(std::shared_ptr<ExecuteSubWfProp::state> s,
...
std::shared_ptr<ExecuteSubWfProp::state>
ExecuteSubWfProp::execute(std::shared_ptr<ExecuteSubWfProp::state> s,
                          const std::shared_ptr<ExecuteSubWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteSubWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteSubWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteSubWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteSubWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteSubWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteSubWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteSubWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteSubWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteSubWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteSubWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            return std::make_shared<ExecuteSubWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteSubWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteSubWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteSubWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
          ...
          [&](const typename ExecuteSubWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteSubWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteSubWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(
          ...
          [&](const typename ExecuteSubWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteSubWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteSubWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteSubWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteSubWfProp::state> {
