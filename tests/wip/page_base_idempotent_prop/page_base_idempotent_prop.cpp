...
#include <memory>
#include <optional>
#include <page_base_idempotent_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>

unsigned int PageBaseIdempotentProp::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int PageBaseIdempotentProp::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int PageBaseIdempotentProp::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int PageBaseIdempotentProp::branch_target(const unsigned int pc,
