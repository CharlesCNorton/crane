...
#include <memory>
#include <optional>
#include <page_decomposition_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>

unsigned int PageDecompositionProp::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int PageDecompositionProp::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int PageDecompositionProp::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int PageDecompositionProp::branch_target(const unsigned int pc,
