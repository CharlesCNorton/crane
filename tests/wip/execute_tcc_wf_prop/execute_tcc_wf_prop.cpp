...
unsigned int
ExecuteTccWfProp::get_reg(const std::shared_ptr<ExecuteTccWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteTccWfProp::set_reg(const std::shared_ptr<ExecuteTccWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteTccWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteTccWfProp::get_pair(const std::shared_ptr<ExecuteTccWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteTccWfProp::set_pair(const std::shared_ptr<ExecuteTccWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteTccWfProp::push_return(std::shared_ptr<ExecuteTccWfProp::state> s,
...
std::shared_ptr<ExecuteTccWfProp::state>
ExecuteTccWfProp::execute(std::shared_ptr<ExecuteTccWfProp::state> s,
                          const std::shared_ptr<ExecuteTccWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteTccWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTccWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteTccWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteTccWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteTccWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteTccWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTccWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTccWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteTccWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteTccWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            return std::make_shared<ExecuteTccWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteTccWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteTccWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteTccWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
          ...
          [&](const typename ExecuteTccWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteTccWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteTccWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(
          ...
          [&](const typename ExecuteTccWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteTccWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteTccWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteTccWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteTccWfProp::state> {
