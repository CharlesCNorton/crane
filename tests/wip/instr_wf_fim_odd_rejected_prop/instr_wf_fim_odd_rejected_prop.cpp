...
#include <cassert>
#include <functional>
#include <instr_wf_fim_odd_rejected_prop.h>
#include <iostream>
#include <memory>
#include <optional>
#include <stdexcept>
...
unsigned int InstrWfFimOddRejectedProp::get_reg(
    const std::shared_ptr<InstrWfFimOddRejectedProp::state> &s,
    const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>> InstrWfFimOddRejectedProp::set_reg(
    const std::shared_ptr<InstrWfFimOddRejectedProp::state> &s,
    const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int InstrWfFimOddRejectedProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int InstrWfFimOddRejectedProp::get_pair(
    const std::shared_ptr<InstrWfFimOddRejectedProp::state> &s,
    const unsigned int r) {
...
std::shared_ptr<List<unsigned int>> InstrWfFimOddRejectedProp::set_pair(
    const std::shared_ptr<InstrWfFimOddRejectedProp::state> &s,
    const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>> InstrWfFimOddRejectedProp::push_return(
    std::shared_ptr<InstrWfFimOddRejectedProp::state> s,
...
std::shared_ptr<InstrWfFimOddRejectedProp::state>
InstrWfFimOddRejectedProp::execute(
    std::shared_ptr<InstrWfFimOddRejectedProp::state> s,
    const std::shared_ptr<InstrWfFimOddRejectedProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename InstrWfFimOddRejectedProp::instr::NOP _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimOddRejectedProp::instr::LDM _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename InstrWfFimOddRejectedProp::instr::LD _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::XCH _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::INC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::ADD _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::SUB _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::IAC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::DAC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::CLC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimOddRejectedProp::instr::STC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimOddRejectedProp::instr::CMC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::CMA _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::CLB _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename InstrWfFimOddRejectedProp::instr::RAL _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::RAR _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::TCC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::TCS _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::DAA _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::KBP _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::JUN _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::JMS _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::JCN _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::FIM _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::SRC _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::FIN _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::JIN _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<InstrWfFimOddRejectedProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::ISZ _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename InstrWfFimOddRejectedProp::instr::BBL _args)
              -> std::shared_ptr<InstrWfFimOddRejectedProp::state> {
