...
unsigned int
ExecuteNopWfProp::get_reg(const std::shared_ptr<ExecuteNopWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteNopWfProp::set_reg(const std::shared_ptr<ExecuteNopWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteNopWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteNopWfProp::get_pair(const std::shared_ptr<ExecuteNopWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteNopWfProp::set_pair(const std::shared_ptr<ExecuteNopWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteNopWfProp::push_return(std::shared_ptr<ExecuteNopWfProp::state> s,
...
std::shared_ptr<ExecuteNopWfProp::state>
ExecuteNopWfProp::execute(std::shared_ptr<ExecuteNopWfProp::state> s,
                          const std::shared_ptr<ExecuteNopWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteNopWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteNopWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteNopWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteNopWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteNopWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteNopWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteNopWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteNopWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteNopWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteNopWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            return std::make_shared<ExecuteNopWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteNopWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteNopWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteNopWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
          ...
          [&](const typename ExecuteNopWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteNopWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteNopWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(
          ...
          [&](const typename ExecuteNopWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteNopWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteNopWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteNopWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteNopWfProp::state> {
