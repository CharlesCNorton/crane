...
#include <any>
#include <cassert>
#include <clb_clears_volatile_prop.h>
#include <functional>
#include <iostream>
#include <memory>
#include <optional>
...
unsigned int ClbClearsVolatileProp::get_reg(
    const std::shared_ptr<ClbClearsVolatileProp::state> &s,
    const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>> ClbClearsVolatileProp::set_reg(
    const std::shared_ptr<ClbClearsVolatileProp::state> &s,
    const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ClbClearsVolatileProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int ClbClearsVolatileProp::get_pair(
    const std::shared_ptr<ClbClearsVolatileProp::state> &s,
    const unsigned int r) {
...
std::shared_ptr<List<unsigned int>> ClbClearsVolatileProp::set_pair(
    const std::shared_ptr<ClbClearsVolatileProp::state> &s,
    const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>> ClbClearsVolatileProp::push_return(
    std::shared_ptr<ClbClearsVolatileProp::state> s, const unsigned int ret) {
  return List<unsigned int>::ctor::cons_((std::move(ret) % 4096u),
                                         std::move(s)->ex_stack)
      ->firstn(2u);
}

std::shared_ptr<ClbClearsVolatileProp::state> ClbClearsVolatileProp::execute(
    std::shared_ptr<ClbClearsVolatileProp::state> s,
    const std::shared_ptr<ClbClearsVolatileProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ClbClearsVolatileProp::instr::NOP _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ClbClearsVolatileProp::instr::LDM _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ClbClearsVolatileProp::instr::LD _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::XCH _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ClbClearsVolatileProp::instr::INC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::ADD _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ClbClearsVolatileProp::instr::SUB _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ClbClearsVolatileProp::instr::IAC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::DAC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::CLC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ClbClearsVolatileProp::instr::STC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ClbClearsVolatileProp::instr::CMC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::CMA _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::CLB _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ClbClearsVolatileProp::instr::RAL _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::RAR _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::TCC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ClbClearsVolatileProp::instr::TCS _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ClbClearsVolatileProp::instr::DAA _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ClbClearsVolatileProp::instr::KBP _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::JUN _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::JMS _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ClbClearsVolatileProp::instr::JCN _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
          ...
          [&](const typename ClbClearsVolatileProp::instr::FIM _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ClbClearsVolatileProp::instr::SRC _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ClbClearsVolatileProp::instr::FIN _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(
          ...
          [&](const typename ClbClearsVolatileProp::instr::JIN _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ClbClearsVolatileProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ClbClearsVolatileProp::instr::ISZ _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ClbClearsVolatileProp::instr::BBL _args)
              -> std::shared_ptr<ClbClearsVolatileProp::state> {
