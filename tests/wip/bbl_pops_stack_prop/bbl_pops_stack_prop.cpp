...
unsigned int
BblPopsStackProp::get_reg(const std::shared_ptr<BblPopsStackProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
BblPopsStackProp::set_reg(const std::shared_ptr<BblPopsStackProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int BblPopsStackProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
BblPopsStackProp::get_pair(const std::shared_ptr<BblPopsStackProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
BblPopsStackProp::set_pair(const std::shared_ptr<BblPopsStackProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
BblPopsStackProp::push_return(std::shared_ptr<BblPopsStackProp::state> s,
...
std::shared_ptr<BblPopsStackProp::state>
BblPopsStackProp::execute(std::shared_ptr<BblPopsStackProp::state> s,
                          const std::shared_ptr<BblPopsStackProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename BblPopsStackProp::instr::NOP _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename BblPopsStackProp::instr::LDM _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename BblPopsStackProp::instr::LD _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::XCH _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename BblPopsStackProp::instr::INC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::ADD _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename BblPopsStackProp::instr::SUB _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename BblPopsStackProp::instr::IAC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::DAC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::CLC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename BblPopsStackProp::instr::STC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename BblPopsStackProp::instr::CMC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::CMA _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::CLB _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename BblPopsStackProp::instr::RAL _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::RAR _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::TCC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename BblPopsStackProp::instr::TCS _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            return std::make_shared<BblPopsStackProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename BblPopsStackProp::instr::DAA _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename BblPopsStackProp::instr::KBP _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::JUN _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::JMS _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename BblPopsStackProp::instr::JCN _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
          ...
          [&](const typename BblPopsStackProp::instr::FIM _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename BblPopsStackProp::instr::SRC _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename BblPopsStackProp::instr::FIN _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(
          ...
          [&](const typename BblPopsStackProp::instr::JIN _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<BblPopsStackProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename BblPopsStackProp::instr::ISZ _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename BblPopsStackProp::instr::BBL _args)
              -> std::shared_ptr<BblPopsStackProp::state> {
