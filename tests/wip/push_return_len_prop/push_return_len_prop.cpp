...
#include <memory>
#include <optional>
#include <push_return_len_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>
...
unsigned int
PushReturnLenProp::get_reg(const std::shared_ptr<PushReturnLenProp::state> &s,
                           const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
PushReturnLenProp::set_reg(const std::shared_ptr<PushReturnLenProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int PushReturnLenProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
PushReturnLenProp::get_pair(const std::shared_ptr<PushReturnLenProp::state> &s,
                            const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
PushReturnLenProp::set_pair(const std::shared_ptr<PushReturnLenProp::state> &s,
                            const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
PushReturnLenProp::push_return(std::shared_ptr<PushReturnLenProp::state> s,
...
std::shared_ptr<PushReturnLenProp::state>
PushReturnLenProp::execute(std::shared_ptr<PushReturnLenProp::state> s,
                           const std::shared_ptr<PushReturnLenProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename PushReturnLenProp::instr::NOP _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnLenProp::instr::LDM _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename PushReturnLenProp::instr::LD _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::XCH _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename PushReturnLenProp::instr::INC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::ADD _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename PushReturnLenProp::instr::SUB _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename PushReturnLenProp::instr::IAC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::DAC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::CLC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnLenProp::instr::STC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnLenProp::instr::CMC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::CMA _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::CLB _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename PushReturnLenProp::instr::RAL _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::RAR _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::TCC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename PushReturnLenProp::instr::TCS _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            return std::make_shared<PushReturnLenProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename PushReturnLenProp::instr::DAA _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename PushReturnLenProp::instr::KBP _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::JUN _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::JMS _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename PushReturnLenProp::instr::JCN _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
          ...
          [&](const typename PushReturnLenProp::instr::FIM _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename PushReturnLenProp::instr::SRC _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename PushReturnLenProp::instr::FIN _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(
          ...
          [&](const typename PushReturnLenProp::instr::JIN _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<PushReturnLenProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename PushReturnLenProp::instr::ISZ _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename PushReturnLenProp::instr::BBL _args)
              -> std::shared_ptr<PushReturnLenProp::state> {
