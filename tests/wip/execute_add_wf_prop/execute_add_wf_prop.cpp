...
unsigned int
ExecuteAddWfProp::get_reg(const std::shared_ptr<ExecuteAddWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteAddWfProp::set_reg(const std::shared_ptr<ExecuteAddWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteAddWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteAddWfProp::get_pair(const std::shared_ptr<ExecuteAddWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteAddWfProp::set_pair(const std::shared_ptr<ExecuteAddWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteAddWfProp::push_return(std::shared_ptr<ExecuteAddWfProp::state> s,
...
std::shared_ptr<ExecuteAddWfProp::state>
ExecuteAddWfProp::execute(std::shared_ptr<ExecuteAddWfProp::state> s,
                          const std::shared_ptr<ExecuteAddWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteAddWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteAddWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteAddWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteAddWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteAddWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteAddWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteAddWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteAddWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteAddWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteAddWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            return std::make_shared<ExecuteAddWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteAddWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteAddWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteAddWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
          ...
          [&](const typename ExecuteAddWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteAddWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteAddWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(
          ...
          [&](const typename ExecuteAddWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteAddWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteAddWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteAddWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteAddWfProp::state> {
