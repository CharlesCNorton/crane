...
unsigned int
ExecuteFinWfProp::get_reg(const std::shared_ptr<ExecuteFinWfProp::state> &s,
                          const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteFinWfProp::set_reg(const std::shared_ptr<ExecuteFinWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteFinWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteFinWfProp::get_pair(const std::shared_ptr<ExecuteFinWfProp::state> &s,
                           const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteFinWfProp::set_pair(const std::shared_ptr<ExecuteFinWfProp::state> &s,
                           const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteFinWfProp::push_return(std::shared_ptr<ExecuteFinWfProp::state> s,
...
std::shared_ptr<ExecuteFinWfProp::state>
ExecuteFinWfProp::execute(std::shared_ptr<ExecuteFinWfProp::state> s,
                          const std::shared_ptr<ExecuteFinWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteFinWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFinWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteFinWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteFinWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteFinWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteFinWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFinWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFinWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteFinWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteFinWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            return std::make_shared<ExecuteFinWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteFinWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteFinWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteFinWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
          ...
          [&](const typename ExecuteFinWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteFinWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteFinWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(
          ...
          [&](const typename ExecuteFinWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteFinWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteFinWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteFinWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteFinWfProp::state> {
