...
#include <memory>
#include <optional>
#include <page_of_inc2_cross_255_prop.h>
#include <stdexcept>
#include <string>
#include <utility>
#include <variant>

unsigned int PageOfInc2Cross255Prop::addr12_of_nat(const unsigned int n) {
  return (n % 4096u);
}

unsigned int PageOfInc2Cross255Prop::page_of(const unsigned int p) {
  return Nat::div(addr12_of_nat(p), 256u);
}

unsigned int PageOfInc2Cross255Prop::page_base(const unsigned int p) {
  return (page_of(p) * 256u);
}

unsigned int PageOfInc2Cross255Prop::branch_target(const unsigned int pc,
