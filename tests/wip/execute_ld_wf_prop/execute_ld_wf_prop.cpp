...
unsigned int
ExecuteLdWfProp::get_reg(const std::shared_ptr<ExecuteLdWfProp::state> &s,
                         const unsigned int r) {
  return s->ex_regs->nth(r, 0u);
}

std::shared_ptr<List<unsigned int>>
ExecuteLdWfProp::set_reg(const std::shared_ptr<ExecuteLdWfProp::state> &s,
                         const unsigned int r, const unsigned int v) {
  return update_nth<unsigned int>(r, (v % 16u), s->ex_regs);
}

unsigned int ExecuteLdWfProp::pair_base(const unsigned int r) {
  return (((r - (r % 2u)) > r ? 0 : (r - (r % 2u))));
}

unsigned int
ExecuteLdWfProp::get_pair(const std::shared_ptr<ExecuteLdWfProp::state> &s,
                          const unsigned int r) {
...
std::shared_ptr<List<unsigned int>>
ExecuteLdWfProp::set_pair(const std::shared_ptr<ExecuteLdWfProp::state> &s,
                          const unsigned int r, const unsigned int v) {
  unsigned int base = pair_base(r);
...
std::shared_ptr<List<unsigned int>>
ExecuteLdWfProp::push_return(std::shared_ptr<ExecuteLdWfProp::state> s,
...
std::shared_ptr<ExecuteLdWfProp::state>
ExecuteLdWfProp::execute(std::shared_ptr<ExecuteLdWfProp::state> s,
                         const std::shared_ptr<ExecuteLdWfProp::instr> &i) {
  return std::visit(
      Overloaded{
          [&](const typename ExecuteLdWfProp::instr::NOP _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
                s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdWfProp::instr::LDM _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int n = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(
                state{(std::move(n) % 16u), s->ex_regs, s->ex_carry,
                      ((s->ex_pc + 1u) % 4096u), s->ex_stack, s->ex_pair_bus,
                      s->ex_ports});
          },
          [&](const typename ExecuteLdWfProp::instr::LD _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::XCH _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteLdWfProp::instr::INC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::ADD _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int sum =
          ...
          [&](const typename ExecuteLdWfProp::instr::SUB _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int diff =
          ...
          [&](const typename ExecuteLdWfProp::instr::IAC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::DAC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::CLC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
                state{s->ex_acc, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdWfProp::instr::STC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
                state{s->ex_acc, s->ex_regs, true, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdWfProp::instr::CMC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::CMA _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::CLB _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
                state{0u, s->ex_regs, false, ((s->ex_pc + 1u) % 4096u),
                      s->ex_stack, s->ex_pair_bus, s->ex_ports});
          },
          [&](const typename ExecuteLdWfProp::instr::RAL _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::RAR _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::TCC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteLdWfProp::instr::TCS _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            return std::make_shared<ExecuteLdWfProp::state>(
                state{[&](void) {
                        if (s->ex_carry) {
          ...
          [&](const typename ExecuteLdWfProp::instr::DAA _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int acc_;
            if ((10u <= (s->ex_acc + 1))) {
          ...
          [&](const typename ExecuteLdWfProp::instr::KBP _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::JUN _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::JMS _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int a = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, (std::move(a) % 4096u),
          ...
          [&](const typename ExecuteLdWfProp::instr::JCN _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
          ...
          [&](const typename ExecuteLdWfProp::instr::FIM _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
          ...
          [&](const typename ExecuteLdWfProp::instr::SRC _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(state{
                s->ex_acc, s->ex_regs, s->ex_carry, ((s->ex_pc + 1u) % 4096u),
          ...
          [&](const typename ExecuteLdWfProp::instr::FIN _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(
          ...
          [&](const typename ExecuteLdWfProp::instr::JIN _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            return std::make_shared<ExecuteLdWfProp::state>(
                state{s->ex_acc, s->ex_regs, s->ex_carry,
          ...
          [&](const typename ExecuteLdWfProp::instr::ISZ _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
            unsigned int r = _args._a0;
            unsigned int a = _args._a1;
          ...
          [&](const typename ExecuteLdWfProp::instr::BBL _args)
              -> std::shared_ptr<ExecuteLdWfProp::state> {
