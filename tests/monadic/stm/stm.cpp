...
void stm_enqueue(
  ...
  return stm::writeTVar<std::shared_ptr<list::list<unsigned int>>>(
      q, app<unsigned int>(xs, list::cons<unsigned int>::make(
                                   x, list::nil<unsigned int>::make())));
}
...
unsigned int stm_dequeue(
  ...
  std::shared_ptr<list::list<unsigned int>> xs =
      stm::readTVar<std::shared_ptr<list::list<unsigned int>>>(q);
  return std::visit(
      Overloaded{[&](const list::nil<unsigned int> _args) -> unsigned int {
                   return stm::retry<unsigned int>();
                 },
                 [&](const list::cons<unsigned int> _args) -> unsigned int {
