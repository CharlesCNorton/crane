...
int main() {
  // Test 1: test_is_nothing - empty_maybe is Nothing, so should be true
  {
    ...
    ASSERT(result == true);
    std::cout << "Test 1 (is_nothing): " << (result ? "true" : "false")
              << " (expected true): PASSED" << std::endl;
  }
  ...
  // Test 2: test_list_len - sample_list has 2 elements (Just 42, Nothing)
  {
    ...
    ASSERT(result == 2);
    std::cout << "Test 2 (list length): " << result << " (expected 2): PASSED"
              << std::endl;
  }
  ...
  // Test 3: test_tree_size - sample_tree is Node with 2 children
  {
    ...
    ASSERT(result == 2);
    std::cout << "Test 3 (tree size): " << result << " (expected 2): PASSED"
              << std::endl;
  }
