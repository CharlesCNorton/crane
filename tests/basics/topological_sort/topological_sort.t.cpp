...
int main() {
  ...
  shared_ptr<list::list<shared_ptr<prod::prod<int, int>>>> g1 =
    list::cons<shared_ptr<prod::prod<int, int>>>::make(prod::pair<int,
  int>::make(0, 1), list::cons<shared_ptr<prod::prod<int,
  int>>>::make(prod::pair<int, int>::make(0, 2),
    list::cons<shared_ptr<prod::prod<int, int>>>::make(prod::pair<int,
  int>::make(1, 2), list::nil<shared_ptr<prod::prod<int, int>>>::make())));
  ...
  std::shared_ptr<list::list<std::shared_ptr<list::list<int>>>> r1 =
    topological_sort<int>(std::move([](int a, int b) { return a == b; }),
  std::move(g1));

  std::cout
      << list_to_string<std::shared_ptr<list::list<int>>>(std::move(
            [](std::shared_ptr<list::list<int>> l) {
                return list_to_string<int>(std::move([](int x) { return
  std::to_string(x); }), std::move(l));
            }), std::move(r1))
      << std::endl; */
