...
unsigned int length(const std::shared_ptr<list::list<T1>> l) {
  return std::visit(
      Overloaded{[&](const list::nil<T1> _args) -> unsigned int { return 0; },
...
concat(const std::shared_ptr<list::list<std::shared_ptr<list::list<T1>>>> l) {
  return std::visit(
      Overloaded{
          [&](const list::nil<std::shared_ptr<list::list<T1>>> _args)
              -> std::shared_ptr<list::list<T1>> {
            return list::nil<T1>::make();
          },
          [&](const list::cons<std::shared_ptr<list::list<T1>>> _args)
              -> std::shared_ptr<list::list<T1>> {
...
T1 fold_right(F0 &&f, const T1 a0, const std::shared_ptr<list::list<T2>> l) {
  return std::visit(
      Overloaded{[&](const list::nil<T2> _args) -> T1 { return a0; },
...
std::optional<T1> find(F0 &&f, const std::shared_ptr<list::list<T1>> l) {
  return std::visit(
      Overloaded{[&](const list::nil<T1> _args) -> std::optional<T1> {
                   return std::nullopt;
                 },
                 [&](const list::cons<T1> _args) -> std::optional<T1> {
                   T1 x = _args._a0;
...
template <typename node> using graph = std::shared_ptr<list::list<entry<node>>>;
...
get_elems(F0 &&eqb_node,
  ...
  get_elems_aux = [&](std::shared_ptr<list::list<std::pair<T1, T1>>> l0,
    ...
    return std::visit(
        Overloaded{
            [&](const list::nil<std::pair<T1, T1>> _args)
                -> std::shared_ptr<list::list<T1>> { return h; },
            [&](const list::cons<std::pair<T1, T1>> _args)
                -> std::shared_ptr<list::list<T1>> {
                ...
                if (f2.has_value()) {
                  ...
                  if (eqb_node(e1, e2)) {
                    ...
                    return get_elems_aux(
                        l_,
                        list::cons<T1>::make(e1, list::cons<T1>::make(e2, h)));
                  }
                }
              }
            }},
...
template <typename T1, MapsTo<bool, T1, T1> F0>
entry<T1> make_entry(F0 &&eqb_node,
                     const std::shared_ptr<list::list<std::pair<T1, T1>>> l,
                     const T1 e) {
...
template <typename T1, MapsTo<bool, T1, T1> F0>
graph<T1> make_graph(F0 &&eqb_node,
                     const std::shared_ptr<list::list<std::pair<T1, T1>>> l) {
  ...
  return fold_right<std::shared_ptr<list::list<entry<T1>>>, T1>(
      [&](T1 e, std::shared_ptr<
                    ...
                    ret) {
        return list::cons<std::pair<T1, std::shared_ptr<list::list<T1>>>>::make(
            make_entry<T1>(eqb_node, l, e), ret);
      },
      list::nil<std::pair<T1, std::shared_ptr<list::list<T1>>>>::make(), elems);
}
...
T1 cycle_entry_aux(
  ...
  if (contains<T1>(eqb_node, elem, seens)) {
    ...
    if (counter <= 0) {
      ...
      return std::visit(
          Overloaded{[&](const list::nil<T1> _args) -> T1 { return elem; },
                     [&](const list::cons<T1> _args) -> T1 {
                       T1 e_ = _args._a0;
                       return cycle_entry_aux<T1>(
                           eqb_node, graph0, list::cons<T1>::make(elem, seens),
                           e_, c);
                     }},
          *l);
    }
...
cycle_entry(F0 &&eqb_node,
  ...
  return std::visit(
      Overloaded{
          [&](const list::nil<std::pair<T1, std::shared_ptr<list::list<T1>>>>
                  _args) -> std::optional<T1> { return std::nullopt; },
          [&](const list::cons<std::pair<T1, std::shared_ptr<list::list<T1>>>>
            ...
            return std::make_optional<T1>(
                cycle_entry_aux<T1>(eqb_node, graph0, list::nil<T1>::make(), e,
...
template <typename T1, MapsTo<bool, T1, T1> F0>
std::shared_ptr<list::list<T1>> cycle_extract_aux(
    F0 &&eqb_node,
    const std::shared_ptr<
        list::list<std::pair<T1, std::shared_ptr<list::list<T1>>>>>
        graph0,
...
template <typename T1> bool null(const std::shared_ptr<list::list<T1>> xs) {
  return std::visit(
      Overloaded{[&](const list::nil<T1> _args) -> bool { return true; },
                 [&](const list::cons<T1> _args) -> bool { return false; }},
      *xs);
}
...
order<T1> topological_sort_aux(
    ...
    const unsigned int counter) {
  if (counter <= 0) {
    return list::nil<std::shared_ptr<list::list<T1>>>::make();
  } else {
    unsigned int c = counter - 1;
          ...
          rest_ = map<std::pair<T1, std::shared_ptr<list::list<T1>>>,
              ...
              [&](std::pair<T1, std::shared_ptr<list::list<T1>>> entry0) {
                return std::make_pair(
                    entry0.first,
                    filter<T1>(
                        [&](T1 e) { return !contains<T1>(eqb_node, e, mins_); },
                        entry0.second));
              },
              rest);
      return list::cons<std::shared_ptr<list::list<T1>>>::make(
...
order<T1> topological_sort_graph(
    ...
    const std::shared_ptr<
        list::list<std::pair<T1, std::shared_ptr<list::list<T1>>>>>
        graph0) {
  return topological_sort_aux<T1>(eqb_node, graph0, length<entry<T1>>(graph0));
}

template <typename T1, MapsTo<bool, T1, T1> F0>
std::shared_ptr<list::list<std::pair<T1, unsigned int>>> topological_rank_list(
    F0 &&eqb_node,
    const std::shared_ptr<
        list::list<std::pair<T1, std::shared_ptr<list::list<T1>>>>>
        graph0) {
  ...
  return concat<std::pair<T1, unsigned int>>(
      map<std::pair<std::shared_ptr<list::list<T1>>, unsigned int>,
          ...
          combine<std::shared_ptr<list::list<T1>>, unsigned int>(
              lorder,
              seq(0, length<std::shared_ptr<list::list<T1>>>(lorder)))));
}
