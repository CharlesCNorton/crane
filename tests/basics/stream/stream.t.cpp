...
int main() {
  ...
  auto vec = list_to_vec<std::shared_ptr<Nat::nat>>(nats_list);
  ASSERT(vec.size() == 5);
  for (int i = 0; i < 5; i++) {
  ...
  auto vec_ones = list_to_vec<std::shared_ptr<Nat::nat>>(ones_list);
  ...
  auto vec_i = list_to_vec<std::shared_ptr<Nat::nat>>(interleaved_list);
  ASSERT(vec_i.size() == 8);
  ...
  auto vec10 = list_to_vec<std::shared_ptr<Nat::nat>>(ten_list);
  ASSERT(vec10.size() == 3);
  ...
  auto vec5 = list_to_vec<std::shared_ptr<Nat::nat>>(five_list);
  ASSERT(vec5.size() == 4);
  for (int i = 0; i < 4; i++) {
  ...
  auto vec_m = list_to_vec<std::shared_ptr<Nat::nat>>(mixed_list);
  ASSERT(vec_m.size() == 6);
