...
int main() {
  ...
  // Test 4: ptree_size
  {
    ASSERT(PolyInductive::test_ptree ==
           5); // PNode(PLeaf, PNode(PLeaf, PLeaf)) = 1+1+1+1+1
    std::cout << "Test 4 (ptree_size): PASSED" << std::endl;
  }

  // NOTE: test_pbox excluded — punbox() returns `this` instead of contained
  // value NOTE: test_pmap excluded — pmaybe_map lambda uses `axiom` type
