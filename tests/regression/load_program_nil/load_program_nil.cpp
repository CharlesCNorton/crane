#include <algorithm>
#include <any>
#include <cassert>
#include <functional>
#include <iostream>
#include <load_program_nil.h>
#include <memory>
#include <optional>
#include <stdexcept>
#include <string>
#include <variant>

std::shared_ptr<LoadProgramNil::state>
LoadProgramNil::set_prom_params(std::shared_ptr<LoadProgramNil::state> s,
                                const unsigned int addr,
                                const unsigned int data, const bool enable) {
  return std::make_shared<LoadProgramNil::state>(state{
      std::move(s)->rom, std::move(addr), std::move(data), std::move(enable)});
}

std::shared_ptr<LoadProgramNil::state>
LoadProgramNil::execute_wpm(std::shared_ptr<LoadProgramNil::state> s) {
  std::shared_ptr<List<unsigned int>> new_rom;
  if (s->prom_enable) {
    new_rom = update_nth<unsigned int>(s->prom_addr, s->prom_data, s->rom);
  } else {
    new_rom = std::move(s)->rom;
  }
  return std::make_shared<LoadProgramNil::state>(
      state{std::move(new_rom), s->prom_addr, s->prom_data, s->prom_enable});
}

std::shared_ptr<LoadProgramNil::state>
LoadProgramNil::load_program(std::shared_ptr<LoadProgramNil::state> s,
                             const unsigned int base,
                             const std::shared_ptr<List<unsigned int>> &bytes) {
  return std::visit(
      Overloaded{[&](const typename List<unsigned int>::nil _args)
                     -> std::shared_ptr<LoadProgramNil::state> {
                   return std::move(s);
                 },
                 [&](const typename List<unsigned int>::cons _args)
                     -> std::shared_ptr<LoadProgramNil::state> {
                   unsigned int b = _args._a0;
                   std::shared_ptr<List<unsigned int>> rest = _args._a1;
                   std::shared_ptr<LoadProgramNil::state> s_ =
                       set_prom_params(std::move(s), base, std::move(b), true);
                   std::shared_ptr<LoadProgramNil::state> s__ =
                       execute_wpm(std::move(s_));
                   return load_program(std::move(s__), (base + 1u),
                                       std::move(rest));
                 }},
      bytes->v());
}
