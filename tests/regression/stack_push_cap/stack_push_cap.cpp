...
#include <memory>
#include <optional>
#include <stack_push_cap.h>
#include <stdexcept>
#include <string>
#include <variant>

std::shared_ptr<StackPushCap::state>
StackPushCap::push_stack(const std::shared_ptr<StackPushCap::state> &s,
  ...
  std::shared_ptr<List<unsigned int>> new_stack = std::visit(
      ...
      s->stack->v());
  return std::make_shared<StackPushCap::state>(state{std::move(new_stack)});
}
