  ...
  return std::visit(
      Overloaded{
          ...
          [](const typename NestedInd::expr::Add _args) -> unsigned int {
            std::shared_ptr<List::list<std::shared_ptr<NestedInd::expr>>> es =
                _args._a0;
            std::function<unsigned int(
                std::shared_ptr<List::list<std::shared_ptr<NestedInd::expr>>>)>
                sum_all;
            sum_all = [&](std::shared_ptr<
                          List::list<std::shared_ptr<NestedInd::expr>>>
                              l) -> unsigned int {
              return std::visit(
